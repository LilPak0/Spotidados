...
export default function useArtistStats(artistName: string) {
  ...
  // Filter tracks for the given artist
  const artistTracks = tracks.filter(
    (track) => track.artist && track.artist.toLowerCase() === artistName.toLowerCase()
  );
  ...
  const differentTracks = new Set(artistTracks.map((track) => track.trackName)).size;
  ...
  const percent = tracks.length > 0 ? Math.round((totalSongs / tracks.length) * 100 * 100) / 100 : 0;
